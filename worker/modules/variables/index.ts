...
export class VariableModule {
	...
	async initVariable({ uidFlow }: { uidFlow: string }) {
		const projectVariables = await this.el.communicationModule.sendDataToServer('workflowProject', {
			uidFlow
		})
		...
		const variables = await this.el.communicationModule.sendDataToServer('workflowVariables', { uidFlow })
		...
		for (const value of this.el.nodeModule.dependencies.credentials.values()) {
			...
			const credentialsResult = await this.el.communicationModule.sendDataToServer('workflowCredentialsResult', { type, name })
	...
	async checkWorkflowEnvironment({ flow }: { flow: IWorkflow }) {
		...
		// Variables
		for (const name of Object.keys(flow.env?.variables || {})) {
	...
	async virtualSecretsAndCredentials(type: string, properties: IPropertiesType) {
		...
		if (propertyCredentials.length > 0) {
			const credentials = await this.el.communicationModule.sendDataToServer('workflowCredentials', { type: type })
