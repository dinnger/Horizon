...
export function subscriberHelper() {
	return {
		send: (id: SubscriberType, value: any, verifyTTL = false) => {
		...
		removeSubscriber: (id: SubscriberType) => {
			if (notificationsList[id]) notificationsList[id] = []
		},
		subscriber: (type: SubscriberType, fn: ({event,data}: {event: SubscriberType,data: any}) => void) => {
...
/**
 * Renders a collection of nodes on a canvas.
 *
 * @param {Object} params - The parameters for rendering nodes.
 * @param {Record<string, Node>} params.nodes - A record of nodes to be rendered.
 * @param {Set<string>} params.selectedNode - A set of node IDs that are selected.
 * @param {Theme} params.theme - The theme to be used for rendering.
...
export function renderNodes({
	...
	for (const node of Object.values(nodes)) {
		const selected = selectedNode.has(node.id)
		render_node({
			ctx,
			theme,
			node,
...
function render_node({
	...
	ctx.lineWidth = 3
	ctx.roundRect(node.x + 4, node.y + 4, node.width - 8, node.height - 8, 6)
	ctx.stroke()
	ctx.lineWidth = 0
	ctx.closePath()
	...
	ctx.fillText(node.icon, node.x + 25, node.y + 56)
	// name
	ctx.font = '12px "Comfortaa Variable"'
	...
	if (node.info) {
		...
		ctx.fillText(
			`${node.info.inputs.length}/${node.info.outputs.length}`,
...
function render_inputs({
	...
	for (const [key, input] of Object.entries(node.inputs)) {
		...
		ctx.roundRect(
			node.x - 5,
			node.y + (25 + Number.parseInt(key) * 20),
			8,
			10,
			2
		)
		if (node.inputs.length > 0) {
			...
			ctx.font = '9px "Comfortaa Variable"'
			ctx.fillText(input, node.x, node.y + (24 + Number.parseInt(key) * 20))
		}
	...
	for (const [key, output] of Object.entries(node.outputs)) {
		...
		if (node.outputs.length > 0) {
			...
			ctx.fillText(
				output,
				node.x + node.width,
				node.y + (24 + Number.parseInt(key) * 20)
			)
		}
		ctx.fillStyle = node.color
		ctx.fill()
...
export function renderAnimation(
	...
	for (const item of animationList) {
		...
		for (const connection of node.connections.filter(
			(f) => f.output === item.outputName && f.id_node_origin === item.id
		)) {
 ...
 * @param {CanvasRenderingContext2D} params.ctx - The canvas rendering context.
 */
function event_selected_nodes({
	selectedNode,
	canvasSelect,
	canvasRelativePos,
	nodes
}: Event_Select_Nodes_Interface) {
	if (!canvasSelect.show) return
	// standarize
	...
	for (const node of Object.values(nodes)) {
		if (
			...
			selectedNode.set(node.id, {
				node,
				relative_pos: {
					x: canvasRelativePos.x - node.x,
					y: canvasRelativePos.y - node.y
				}
			})
		}
	}
...
export function drawNodeConnectionPreview({
	...
	node_connection_new: { node: INode; output_index: number }
	canvasRelativePos: { x: number; y: number }
	nodes: { [key: string]: INodeCanvas }
	ctx: CanvasRenderingContext2D
}) {
	const node_origin = node_connection_new.node
	...
	if (node_destiny?.node && node_destiny.input_index !== null) {
		setTempConnection({
			...
			output:
				node_connection_new.node.outputs[node_connection_new.output_index],
	...
	const bezier_value = calcBezier({
		...
		destiny: {
			...
			y:
				node_destiny?.node && node_destiny.input_index !== null
					? node_destiny.node.y + (30 + node_destiny.input_index * 20)
					: canvasRelativePos.y
		},
		output_index: node_connection_new.output_index
	})
	draw_bezier({
		ctx,
...
export function renderConnectionNodes({
	...
	if (!connection.pointers) {
		const shapeA = {
			left: node_origin.x,
			top: node_origin.y,
			width: node_origin.width,
			height: node_origin.height
		}
		const shapeB = {
			left: node_destiny.x,
			top: node_destiny.y,
			width: node_destiny.width,
			height: node_destiny.height
		}
		connector = OrthogonalConnector.route({
			...
			pointA: {
				shape: shapeA,
				side: 'right',
				distance: 30 + node_origin.outputs.indexOf(output) * 20
			},
			pointB: {
				shape: shapeB,
				side: 'left',
				distance: 30 + node_destiny.inputs.indexOf(input) * 20
			},
			shapeMargin: 5,
		...
		if (firstPoint) {
			...
			color.addColorStop(0, node_origin.color || '#3498DB')
			color.addColorStop(1, node_destiny.color || '#3498DB')
	...
	// 	destiny: {
	// 		x: node_destiny.x - 9,
	// 		y: node_destiny.y + (30 + node_destiny.inputs.indexOf(input) * 20)
	// 	},
	// 	output_index: node_origin.outputs.indexOf(output),
	// 	time: 0.5
	// })

	// connection.pos_property.pos = {
	// 	x: bezier_value.POS_T?.x || 0,
	// 	y: bezier_value.POS_T?.y || 0
	// }

	// if (connection.pos_property.show && connection.pos_property.pos) {
	// 	ctx.beginPath()
	// 	ctx.fillStyle = color
	// 	ctx.strokeStyle = '#333'
	// 	ctx.arc(
	// 		connection.pos_property.pos.x,
	// 		connection.pos_property.pos.y,
	// 		10,
	// 		0,
	// 		2 * Math.PI
	// 	)
...
function calcBezier({
	...
	const originX = node_origin.x + node_origin.width + 9
	const originY = node_origin.y + (30 + output_index * 20)
...
export function verifyNodeFocus({
	...
	for (const node of Object.values(nodes)) {
		if (
			node.x < x + x_margin &&
			node.x + node.width + x_margin2 > x &&
			node.y < y + y_margin &&
			node.y + node.height + y_margin2 > y
		) {
			nodes_selected = node
			for (const input of Object.keys(node.inputs)) {
				if (
					node.x + x_margin > x &&
					node.y + 25 + Number.parseInt(input) * 20 - 5 < y &&
					node.y + 25 + Number.parseInt(input) * 20 + 15 > y
				) {
			...
			for (const output of Object.keys(node.outputs)) {
				if (
					node.x + node.width < x &&
					node.y + 25 + Number.parseInt(output) * 20 - 5 < y &&
					node.y + 25 + Number.parseInt(output) * 20 + 15 > y
				) {
