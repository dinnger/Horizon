...
interface IProperties extends INodeClassProperty {
	...
	authMode: Extract<INodeClassPropertyType, { type: 'options' }> // 'secret' or 'manual'
	authSecret: Extract<INodeClassPropertyType, { type: 'credential' }> // Used if authMode is 'secret'
	host: Extract<INodeClassPropertyType, { type: 'string' }> // Used if authMode is 'manual'
	port: Extract<INodeClassPropertyType, { type: 'number' }> // Used if authMode is 'manual'
	password: Extract<INodeClassPropertyType, { type: 'password' }> // Used if authMode is 'manual', optional
	key: Extract<INodeClassPropertyType, { type: 'string' }>
	value: Extract<INodeClassPropertyType, { type: 'string' }> // Used for SET operation
	db: Extract<INodeClassPropertyType, { type: 'number' }> // Redis DB number
}
...
interface ICredentials extends INodeClassProperty {
	...
	password: Extract<INodeClassPropertyType, { type: 'password' }> // Optional
	db: Extract<INodeClassPropertyType, { type: 'number' }>
