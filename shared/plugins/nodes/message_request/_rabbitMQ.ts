...
export default class implements INodeMicroservice {
	...
	constructor({
		...
		execute,
		outputData
	}: { context: INodeMicroservice['context']; execute: INodeMicroservice['execute']; outputData: INodeMicroservice['outputData'] }) {
		this.outputData = outputData
		this.execute = execute
		this.context = context

		if (!context.project || context.project.type !== 'rabbitMQ') return
		const { host, exchange, maxRetries } = context.project.rabbitMQ
		this.host = host
