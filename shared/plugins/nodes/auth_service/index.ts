...
export default class AuthServiceNode implements INodeClass {
	...
	async onCreate({ dependency, environment }: Parameters<NonNullable<INodeClass['onCreate']>>[0]) {
			...
			switch (authMethod) {
				case 'oauth2':
					...
					if (!this.properties.redirectUri.value) {
						this.properties.redirectUri.value = `${environment.SERVER_URL}/auth/callback`
					}
					break
